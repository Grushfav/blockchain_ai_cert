...
class OperationsDigestMetricsTests(unittest.TestCase):
    ...
    def test_single_mint_student_claim_request_uses_certificate_record(self) -> None:
        ...
        cert = CertificateRecord(
            ...
            cert_id="CERT-SM-CLAIM-1",
            ipfs_uri="ipfs://QmSingleMintClaim",
            core_hash="0x" + "a" * 64,
            status="issued",
            student_internal_id="SID-CLAIM-1",
            student_email="student@example.edu",
        )
        db.session.add(cert)
        db.session.commit()
