...
class OperationsDigestMetricsTests(unittest.TestCase):
    ...
    def test_student_claim_request_accepts_single_mint_record(self) -> None:
        ...
        try:
            with patch.object(student_claim_routes.blockchain_service, "escrow_claim_eligibility", return_value=(True, None)):
                r = self.client.post(
            ...
            req = StudentClaimRequest.query.filter_by(token_id=777).first()
            self.assertIsNotNone(req)
            self.assertIsNone(req.mint_batch_row_id)
            self.assertEqual(req.cert_id, "CERT-SINGLE-CLAIM")
            self.assertEqual(req.student_email, "claimant@example.edu")
        finally:
            self._cleanup_mint_fixtures()
