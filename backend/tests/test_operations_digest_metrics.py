    ...
    def test_public_claim_request_accepts_single_mint_certificate(self) -> None:
        ...
        db.session.add(
            CertificateRecord(
                ...
                status="issued",
                student_internal_id="STU-CLAIM-1",
        ...
        try:
            with patch.object(claim_mod.blockchain_service, "escrow_claim_eligibility", return_value=(True, None)):
                with patch.object(claim_mod.notification_service, "notify_university_users", return_value=1):
                    r = self.client.post(
            ...
            data = r.get_json() or {}
            self.assertEqual(data.get("token_id"), 88)
            rec = StudentClaimRequest.query.filter_by(token_id=88).one()
