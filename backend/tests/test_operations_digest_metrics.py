    ...
    def test_public_claim_request_accepts_single_mint_certificate(self) -> None:
        ...
        db.session.add(
            CertificateRecord(
                ...
                core_hash="0x" + "a" * 64,
                status="issued",
                student_internal_id="STU-77",
                student_email="Student@Example.edu",
            )
        )
        db.session.commit()

        with patch("app.student_claim_routes.blockchain_service.escrow_claim_eligibility", return_value=(True, None)):
            with patch("app.student_claim_routes.notification_service.notify_university_users", return_value=1):
                r = self.client.post(
        ...
        req = StudentClaimRequest.query.filter_by(token_id=77).one()
